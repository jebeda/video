...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 22 17:32:07 2020
...
def _parse_filename(filename):
    ...
    person_id, cam_seq, frame_idx = filename_base.split('_')#####
...
def read_train_split_to_str(dataset_dir):
    ...
    #for filename in sorted(glob(image_dir + '/*.jpg')):
        #print(filename)
        ...
        if meta_data is None:
            ...
            continue
        if meta_data[0] not in ids_to_classes:
            ids_to_classes[meta_data[0]] = len(ids_to_classes)
        meta_data[0] = ids_to_classes[meta_data[0]]

        filenames.append(os.path.join(image_dir, filename))
        ids.append(meta_data[0])
        camera_indices.append(meta_data[1])
        #print("###############length of files:",len(filenames))#16552confirmed
    #print("MTMC.py-filenams:",filenames)
...
def read_train_split_to_image(dataset_dir):
    ...
    for i, filename in enumerate(filenames):
        rawimg = cv2.imread(filename, cv2.IMREAD_COLOR)#####
        images[i] = cv2.resize(rawimg,(IMAGE_SHAPE[0],IMAGE_SHAPE[1]))#####
        print(cv2.resize(rawimg,(IMAGE_SHAPE[1],IMAGE_SHAPE[0])).shape)

    ids = np.asarray(ids, np.int64)
    camera_indices = np.asarray(camera_indices, np.int64)
    #print("###############length of files:",len(images))
...
def read_test_split_to_image(dataset_dir):
    ...
    for i, filename in enumerate(gallery_filenames):
        rawgall = cv2.imread(filename, cv2.IMREAD_COLOR)#####
        gallery_images[i] = cv2.resize(rawgall,(IMAGE_SHAPE[0],IMAGE_SHAPE[1]))#####

    query_images = np.zeros((len(query_filenames), 128, 64, 3), np.uint8)
    for i, filename in enumerate(query_filenames):
        rawquery = cv2.imread(filename, cv2.IMREAD_COLOR)#####
        query_images[i] = cv2.resize(rawquery,(IMAGE_SHAPE[0],IMAGE_SHAPE[1]))#####
